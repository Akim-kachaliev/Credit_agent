...
|:------------|:---------|
| Приём файлов | Договор, спецификация, счёт, акт |
| Комплектность | Проверка наличия всех обязательных документов |
| Валидация форматов | PDF, DOCX, JPG, PNG |
| Ограничение размера | Максимальный размер файла — 20 МБ |
| Распознавание типов | Определение типа документа по ключевым словам в имени файла |
| История проверок | Все результаты сохраняются в PostgreSQL |
| Понятный статус | `approved`, `rejected` или `check_in_progress` с деталями |
